...
The easiest way it to use the __create-bridge-app__ npm package.
...
- Create a __vercel.json__ file
- Edit the __index.ts__ file
- Edit the __tsconfig.json__ file
- Edit the __package.json__ file

### vercel.json

The first thing that we will do is to create a __vercel.json__ file at the root of your project.
...
To make it work, we'll also need to update our __index.ts__ file and __initBridge__ on the __/api__ route:
```ts
import {
  initBridge,
  handler,
...
app.use(
  "/api",
  initBridge({ routes }).expressMiddleware()
);
...
Don't forget to add the __module.exports = app__ at the end of your file. 
...
Now everything should be ready for the deploy, just use __vercel__.
