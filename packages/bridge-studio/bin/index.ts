...
//   const packageJson = isBridgeProjectAndGetPackageJSON();
...
//   (async () => {
//     const resToken = await prompts({
//       type: 'text',
//       name: 'token',
//       message: 'Log with your github account and copy your secretToken:',
//       // validate: (token) => (token < 18 ? `Nightclub is 18+ only` : true),
//     });

//     console.log(resToken); // => { value: 24 }

//     const regexp = new RegExp('^[a-z0-9-]+$');

//     const resProjectName = await prompts({
//       type: 'text',
//       name: 'projectName',
//       message: `What's your project's name?`,
//       initial: packageJson.name || '',
//       validate: (projectName) =>
//         regexp.test(projectName) ? true : `You can only use alphanumeric characters and -'`,
//     });
...
//   if (!bridgeConfig) process.exit(1);
...
//   const projectDir = `${tmpDir}/${new Date().getTime()}`;
//   await mkdirAsync(projectDir);
//   const projectTempPath = `${projectDir}/${new Date().getTime()}`;
//   await mkdirAsync(projectTempPath);
//   zip.extractAllTo(projectTempPath);
...
//   const a = await API.project.compileWithCommandLine({
//     file: { projectZipBuffer: newZip.toBuffer() },
//     headers: { token: bridgeConfig.secretToken },
//     query: { serverUrl: bridgeConfig.serverUrl, projectName: bridgeConfig.projectName },
//   });
